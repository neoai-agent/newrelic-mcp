...
NR_INSIGHTS_API_KEY = None
NEW_RELIC_ACCOUNT_ID = None
LLM_MODEL = None
...
class NewRelicClient:
    def __init__(self):
        ...
        self.account_id = NEW_RELIC_ACCOUNT_ID
...
def run_server(new_relic_api_key: str, nr_insights_api_key: str, new_relic_account_id: str, model: str):
    """Run the MCP server with the provided configuration"""
    global NEW_RELIC_API_KEY, NR_INSIGHTS_API_KEY, NEW_RELIC_ACCOUNT_ID, LLM_MODEL, nr_client, _newrelic_applications_available
    
    # Validate required arguments
    if not new_relic_api_key:
        raise ValueError("NEW_RELIC_API_KEY must be provided")
    if not new_relic_account_id:
        raise ValueError("NEW_RELIC_ACCOUNT_ID must be provided")
    if not nr_insights_api_key:
        raise ValueError("NR_INSIGHTS_API_KEY must be provided")
    if not model:
        raise ValueError("Model must be provided")

    # Set global variables
    NEW_RELIC_API_KEY = new_relic_api_key
    NR_INSIGHTS_API_KEY = nr_insights_api_key
...
if __name__ == "__main__":
    raise RuntimeError("This module should be imported and run_server() should be called with arguments.")